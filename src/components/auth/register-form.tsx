...
export function RegisterForm() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-6">
      ...
      {error && (
        <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-xs font-bold flex items-center gap-2.5">
      ...
      {step === 1 && (
        <form onSubmit={handleNextStep1} className="space-y-4">
          <div className="space-y-1">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Owner Full Name</label>
            <div className="relative">
              ...
              <input
                ...
                className="h-11 w-full pl-10 pr-3.5 border border-slate-300 rounded-xl text-xs font-medium outline-none focus:border-teal-700 focus:ring-2 focus:ring-teal-700/10 bg-white"
              />
            </div>
          </div>

          <div className="space-y-1">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Car Wash Store Name</label>
            <div className="relative">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Mobile Number or Email</label>
            <div className="relative">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Create Password</label>
            <div className="relative">
          ...
          <button
            type="submit"
            className="w-full h-12 bg-teal-700 hover:bg-teal-800 text-white rounded-xl font-bold text-xs shadow-md flex items-center justify-center gap-2 transition active-tap mt-4"
          >
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="text-center space-y-1">
            <span className="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px] font-black rounded-full uppercase tracking-wider inline-flex items-center gap-1.5">
              <Sparkles size={13} />
              1 Month Free Trial Included on All Plans
            </span>
            <h3 className="text-base font-black text-slate-800">Select Your Station Plan</h3>
            <p className="text-xs text-slate-500">No credit card required to start. Full feature access for 30 days.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
            ...
            >
              <div>
                ...
                <div className="mt-2">
                  <span className="text-xl font-black text-slate-900">SAR 0</span>
                  <span className="text-[10px] text-slate-400 block font-semibold">1st Month Free (then 199/mo)</span>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-200/60 text-center">
                <span className={`text-[10px] font-extrabold ${selectedPlan === "STARTER" ? "text-teal-700" : "text-slate-400"}`}>
            ...
            <div
              ...
              className={`p-4 rounded-2xl border cursor-pointer transition relative flex flex-col justify-between ${
                selectedPlan === "PRO_STATION"
                  ? "border-teal-600 bg-teal-50/60 ring-2 ring-teal-600/30 shadow-xl"
                  : "border-slate-200 bg-white hover:border-slate-300"
              }`}
            >
              ...
              <div>
                ...
                <div className="mt-2">
                  <span className="text-xl font-black text-slate-900">SAR 0</span>
                  <span className="text-[10px] text-teal-700 block font-extrabold">1st Month Free (then 499/mo)</span>
                </div>
                <ul className="mt-3 space-y-1.5 text-[11px] text-slate-700 font-medium">
              ...
              <div className="mt-4 pt-3 border-t border-teal-200 text-center">
                <span className={`text-[10px] font-black ${selectedPlan === "PRO_STATION" ? "text-teal-800" : "text-slate-400"}`}>
            ...
            >
              <div>
                ...
                <div className="mt-2">
                  <span className="text-xl font-black text-slate-900">SAR 0</span>
                  <span className="text-[10px] text-slate-400 block font-semibold">1st Month Free (then 999/mo)</span>
                </div>
                <ul className="mt-3 space-y-1.5 text-[11px] text-slate-600">
              ...
              <div className="mt-4 pt-3 border-t border-slate-200/60 text-center">
                <span className={`text-[10px] font-extrabold ${selectedPlan === "ENTERPRISE" ? "text-teal-700" : "text-slate-400"}`}>
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={() => setStep(1)}
              className="h-12 px-5 border border-slate-300 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-50 transition"
            >
              <ArrowLeft size={16} />
            </button>
            <button
      ...
      {step === 3 && (
        <form onSubmit={handleRegisterSubmit} className="space-y-5">
          <div className="text-center space-y-1">
            <h3 className="text-base font-black text-slate-800">Set Initial Wash Prices (SAR)</h3>
            <p className="text-xs text-slate-500">Specify your station's wash pricing for different vehicle categories below. You can change these anytime in Settings.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            {/* Bike */}
            <div className="p-3.5 border border-slate-200 rounded-xl bg-slate-50/60 flex items-center justify-between">
              <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  className="w-20 h-9 border border-slate-300 rounded-lg px-2 text-xs font-extrabold text-slate-800 text-right bg-white outline-none focus:border-teal-700"
                />
              </div>
            </div>
            ...
            {/* Luxury */}
            <div className="p-3.5 border border-slate-200 rounded-xl bg-slate-50/60 flex items-center justify-between sm:col-span-2">
              <div>
