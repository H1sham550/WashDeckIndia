...
export async function POST(
  ...
  try {
    ...
    // Prepare PDF data
    const subtotal = job.invoice ? Number(job.invoice.subtotal) : job.services.reduce((sum: number, s: any) => sum + Number(s.priceSnapshot), 0);
    ...
    const pdfBuffer = generateReportPDF({
      ...
      stationAddress: job.station.branding?.businessAddress || undefined,
      stationPhone: job.station.branding?.businessPhone || undefined,
      stationEmail: job.station.branding?.businessEmail || undefined,
      ...
      services: job.services.map((s: any) => ({ name: s.serviceNameSnapshot, price: Number(s.priceSnapshot) })),
      subtotal,
      discount,
      finalAmount,
